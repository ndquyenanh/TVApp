...
/**
 * Created by Qnv96 on 12-Jan-17.
 * Start application when device booted complete
 */

public class BootReceiver extends BroadcastReceiver {

    public BootReceiver() {
        Debug.log("Start application after booted device!!!");
    }
    ...
    public void onReceive(Context context, Intent intent) {
        Debug.log("receive booted!!!");
        if (SettingManager.isLaunchWithDevice(context)){
            Intent i = new Intent(context, MainActivity.class);
            i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            i.putExtra(Define.BOOT_COMPLETE, Define.BOOT_OK);
